...
def setup_chat_routes(
    ...
    async def chat_endpoint(request: Request, chat_request: ChatRequest) -> Dict[str, Any]:
        ...
        if use_research and not research_blocked_by_policy:
            try:
                ...
                # Appended after the request, not inserted before it, so the
                # stable prompt prefix stays cacheable (append_dynamic_context).
                ctx.messages = append_dynamic_context(ctx.messages, [research_message])
                if foreground_policy.enabled and getattr(ctx, "route_messages", None):
                    # The route-neutral copy is what a fallback candidate sends.
                    ctx.route_messages = append_dynamic_context(
                        ctx.route_messages, [research_message]
                    )
            except Exception as e:
    ...
    async def chat_stream(request: Request) -> StreamingResponse:
        ...
        # Through stored_disabled_tools(), not the raw `disabled_tools` field:
        # a chat narrowed by an allowlist stores `tool_access`/`enabled_tools`
        # and leaves `disabled_tools` empty, so reading the field directly sees
        # an unrestricted chat and admits everything the allowlist excludes.
        ...
        _detached_run = agent_runs.start(session, _safe_stream(), owner=effective_user(request))
        return StreamingResponse(
