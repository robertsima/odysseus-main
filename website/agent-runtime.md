...
A loadout with `tool_access="selected"` is stored as an allowlist and inverted
where it is evaluated, so by the time `stream_agent_loop` has built its
effective `disabled_tools` the allowlist is readable back out of it:
`_pinned_policy_toolset` does exactly that and binds the whole set whenever it
holds no more than `agent_pinned_toolset_max_tools` (25) tools. Retrieval, the
embedding call and domain seeding are skipped, and `_reassert_pinned_toolset`
puts the set back after the shaping passes that run for every selection (§2.2),
so the schema list is byte-identical across the rounds of a turn and across
turns and the cached prefix holds. `[tool-routing] source=pinned` says it
happened.
...
`src/tool_policy.py::allowlist_permits`, applied by the gate table in §2.3, by
`session_settings.stored_disabled_tools` for a chat continuing itself, and again
at execution in `src/tool_execution.py`. Three places used to invert it at save
time instead, each against a different registry (`known_tool_names()`,
...
`disabled_tools` its loadout wrote at the time; explicit denials are still
stored and still subtract, whatever the allowlist says.

`discover_tools` is the one name the inversion does not deny under a non-empty
`selected` allowlist. The execution gate admits it so an agent can read back its
own bindings rather than guess, and the offer has to match the enforcement: a
tool the executor will run must not be missing from every schema list. An
explicit `disabled_tools` entry still overrides that.

### Continuing a chat headlessly
