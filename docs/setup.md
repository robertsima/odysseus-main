...
- **Calendar/contacts (Radicale) won't sync.** Docker Compose includes a local Radicale service. From inside Odysseus, use the compose hostname and the **full collection URL** with its trailing slash: `http://radicale:5232/<user>/<collection-id>/`. Do not use only `http://radicale:5232`. If you open Radicale's web UI through the host port, it shows the collection address after you create a calendar; translate the host part back to `radicale:5232` when pasting it into Odysseus.
