...
- **Calendar/contacts (Radicale) won't sync.** Docker Compose includes a local Radicale service after you recreate the stack with the updated compose file. From Odysseus running in that stack, use the compose hostname and the **full collection URL** with its trailing slash: `http://radicale:5232/odysseus/personal/`. Do not use only `http://radicale:5232`. If Odysseus is running outside Docker Compose, `radicale` will not resolve; use the host/LAN address for the Radicale container instead, e.g. `http://192.168.1.50:5232/odysseus/personal/`, and keep `ODYSSEUS_ALLOW_PRIVATE_CALDAV=1`.
