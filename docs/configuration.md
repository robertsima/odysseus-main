...
| `ODYSSEUS_FASTEMBED_LANE` | `src/embedding_lanes.py:38` | Whether the local FastEmbed (ONNX) fallback embedding lane is built alongside a working custom embedding endpoint (`auto`) or only when that endpoint is down (`off`). | `off` exists specifically to stop paying compute/memory for a lane a *resource-constrained host* doesn't need — a deployment-capacity fact — but it's set by an operator choice, not detected automatically. Leaning CHOICE if forced, matching the RAG-tuning precedent, but a case can be made it belongs next to `EMBEDDING_URL` as deployment wiring. |
