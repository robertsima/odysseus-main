                    ...
                    if evt == "response.output_text.delta":
                        ...
                        if isinstance(usage, dict):
                            ...
                            if normalized_usage and (
                                ...
                                # Prompt-cache effectiveness, which the agent loop logs
                                # per round (`[agent-usage] cached=`).
                                input_details = (
                        ...
                        # Responses delivers tool calls as output items, not as
                        # chat-completions deltas; hand them to the loop in the one
                        # shape it consumes. Without this a Responses-API model (the
                        # ChatGPT subscription lane) could not call a tool at all.
                            ...
                            if data.startswith("{"):
                                ...
                                if _has_genuine_usage and not _delta_has_output:
                                    ...
                                    if isinstance(_input_details, dict):
                                        ...
                                        if isinstance(_cached_tokens, int) and _cached_tokens > 0:
                                            _usage_data["cached_input_tokens"] = _cached_tokens
                                        if isinstance(_cache_write_tokens, int) and _cache_write_tokens > 0:
