...
def crew_disabled_tools(crew) -> Set[str]:
    ...
    The name universe is ``BUILTIN_TOOL_DESCRIPTIONS`` — deliberately the same
    one the scheduler inverted against before this link existed, so a crew
    member that names no profile is scoped byte-for-byte as it was.  MCP tool
    names are not in it and are not invented here: MCP access is the profile's
    ``allowed_mcp_servers``, which is enforced by server rather than by name
    (``docs/design-patterns.md``, "recognise by shape").
    """
    ...
    try:
        ...
        names = set(BUILTIN_TOOL_DESCRIPTIONS.keys())
    except Exception:
        return set()
...
def effective_disabled_tools(crew) -> Set[str]:
    ...
    if patch:
        return set(patch.get("disabled_tools") or ())
