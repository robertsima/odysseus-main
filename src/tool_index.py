...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "inspect_runtime": "Explain why a scheduled task behaved the way it did, and where a configuration value came from. action=tasks lists your scheduled tasks with the scheduler lane each lands in; action=task with a task_id returns the prompt its recent runs actually sent, whether each run succeeded, failed, timed out or was aborted and why, the tool calls those runs made (email tool calls included), the tool/approval policy each ran under, and every time the scheduler decided not to run it; action=config says whether a setting comes from an environment variable, data/settings.json, your own prefs or the code default. Read-only; credentials are never returned.",
