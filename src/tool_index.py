...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "manage_git": "General local Git workflows: discover repositories, status, diffs, commit history/log, branches/remotes, stage and unstage files, commit, create branch/tag, switch branches, fetch, pull, push, fast-forward merge and delete merged branches. Works within approved checkouts without shell or private-vault access. Push/merge/deletion require exact-call human confirmation. GitHub Actions/API metadata cannot update local checkouts. No arbitrary commands, force/reset/rebase or private-vault paths.",
