    ...
    },
    {
        ...
        "function": {
            ...
            "parameters": {
                "type": "object",
                "properties": {
                    "action": {"type": "string", "enum": ["list", "get", "capabilities", "preflight", "create", "update", "delete", "start", "status"], "description": "Default list. capabilities = the ceiling a loadout authored here may reach. preflight = READY/DEGRADED/BLOCKED for a saved loadout right now: each tool's state, selected skills' dependencies, document-index currency, indexed vs raw private access, web lookup, with the repair for each failure. start = launch a worker: REQUIRES 'task' (optionally 'name'), e.g. {\"action\":\"start\",\"name\":\"Auditor\",\"task\":\"Review X and report Y\"}. status = what the workers this chat started actually did, including which ran out of rounds. Use status instead of searching logs or guessing."},
