...
def _run(monkeypatch, rounds, *, disabled_tools=None):
    ...
    events = _events(_collect(al.stream_agent_loop(
        ...
        # "Hand this to a subagent" opens the delegation gate (without it
        # `manage_agent_loadout` is in _DELEGATION_TOOLS and denied outright)
        # while routing only the delegate_* tools, so the tool under test is
        # genuinely absent from round 1. "loadout" would have routed it into
        # the schema list directly -- correct behaviour, but then there is
        # nothing left for the re-arm to demonstrate.
