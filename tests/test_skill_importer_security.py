...
def test_parse_skill_source_rejects_skills_sh_page_that_never_reaches_github():
    ...
    An exact ``skills.sh/<owner>/<repo>/<skill>`` link is mapped without the
    network instead (the skill name selects the folder), so this is exercised
    with a link that does not name a skill.
    """
    body = '<html><body><a href="https://github.com/test-owner/test-repo">Repository</a></body></html>'
    with patch("services.memory.skill_importer._get_checked") as mock_get:
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.url = "https://www.skills.sh/anthropics/skills"
