...
def test_scheduler_fallbacks_and_research_headers_are_owner_scoped():
    ...
    assert (
        "def _resolve_endpoint_headers(self, endpoint_url: str, owner: str | None"
        in src
    )
    assert re.search(r"endpoint_runtime_headers\(\w+,\s*owner=owner or None\)", src), (
        "the owner scope must reach runtime header/credential resolution"
    )
