...
def test_round_usage_is_also_yielded_as_a_frame(monkeypatch):
    """The same numbers reach a detached run's Workbench row, not just the log."""
    monkeypatch.setattr(agent_loop, "get_setting", lambda key, default=None: default)
    monkeypatch.setattr(agent_loop, "get_mcp_manager", lambda: None)
    monkeypatch.setattr(agent_loop, "_classify_agent_request", lambda messages, latest: {
        "low_signal": False, "continuation": False, "domains": [], "retrieval_query": latest})
    ...
    frames = [json.loads(c[6:]) for c in chunks if c.startswith("data: {") and '"round_usage"' in c]
    assert frames == [{"type": "round_usage", "round": 1, "input": 12000, "cached": 9000, "output": 40}]
